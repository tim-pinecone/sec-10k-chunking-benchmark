...
**Additional issue — LateChunker results are specifically understated (see note below).**
...
### 7. LateChunker was tested with the wrong embedding model

Late chunking is a technique invented by Jina AI and designed specifically for long-context encoder models that use mean pooling (e.g. `jina-embeddings-v2`, `jina-embeddings-v3`, `nomic-embed`). The method works by running the full document through the model in one forward pass, then applying per-chunk mean pooling over the resulting token embeddings — so each chunk's embedding carries context from the entire document.

Our benchmark ran LateChunker against `text-embedding-ada-002`, which was not designed for this approach. Ada-002 is an OpenAI proprietary model with no published architecture, and it is not a mean-pooled encoder in the same sense. Chonkie's LateChunker almost certainly fell back to standard per-chunk encoding when paired with this model, meaning the contextual pooling benefit was not actually exercised.

There is a second structural problem: SEC 10-K filings are 50,000–150,000 tokens. Even the Jina models that late chunking was designed for have an 8,192-token context window. A full 10-K cannot fit in one pass, so LateChunker must process the document in windows. This breaks the core assumption — cross-chunk context is only shared within a window, not across the full filing. The benefit that makes late chunking compelling (e.g. a chunk about "its population" knowing it refers to "Berlin" two sentences earlier) is limited to within-window spans.

**Consequence for results:** LateChunker's R@10 of 50.1% is almost certainly an underestimate of what it could achieve in a fair test. The fair comparison would be: LateChunker + `jina-embeddings-v3` vs RecursiveChunker + `jina-embeddings-v3`, on a corpus where documents fit within the model's context window.

**Fix:** Re-run LateChunker with `jina-embeddings-v3` (or another Jina long-context model). For the SEC corpus specifically, consider truncating documents to the model's context window or using a model with a larger window (e.g. 32K+) before drawing conclusions about LateChunker's ceiling.

**Reference:** [What Late Chunking Really Is and What It's Not — Part II](https://jina.ai/news/what-late-chunking-really-is-and-what-its-not-part-ii/)

---

### 8. Chunk overlap not tested
...
The finding that **larger chunk sizes outperform smaller ones** is robust. The finding that **RecursiveChunker is the best specific strategy** is plausible but not conclusive for two reasons: (1) the margin over LateChunker is within noise, and (2) LateChunker was not tested under conditions where it can actually work — it requires a Jina long-context mean-pooling model, not ada-002, and documents must fit within the model's context window. LateChunker's results in this benchmark should be read as a lower bound, not a ceiling.

The strongest defensible claim is: **use chunk_size=1024 with RecursiveChunker as a safe, fast default for ada-002**. Do not pay the cost of SemanticChunker or NeuralChunker for marginal gains on this embedding model. LateChunker warrants a dedicated re-run with a Jina model before any conclusion can be drawn about its true performance.
